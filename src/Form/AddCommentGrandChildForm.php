<?php
/**
 * @file
 * Contains Drupal\kozon\Form\AddCommentGrandChildForm
 */

namespace Drupal\kozon\Form;

use Drupal\Core\Form\FormBase;
use Drupal\Core\Form\FormStateInterface;
use Drupal\kozon\HeroStorage;
use Drupal\Core\Url;
use Drupal\Component\Utility\UrlHelper;
use Drupal\Core\StringTranslation\StringTranslationTrait;
use Drupal\Core\Entity\EntityBase;
use Drupal\file\Entity\File;

/**
 * Class AddForm.
 *
 * @package Drupal\kozon\Form\AddForm
 */
class AddCommentGrandChildForm extends FormBase {

  use StringTranslationTrait;

  protected $account;

  public function __construct(){
    $this->account = \Drupal::currentUser();
  }

  /**
   * {@inheritdoc}
   */
  public function getFormId() {
    return 'kozon_comment_add_child';
  }

  /**
   * {@inheritdoc}
   */
  public function buildForm(array $form, FormStateInterface $form_state,$arg = NULL) {
    global $base_url;
    
    $form = array(
        '#attributes' => array('enctype' => 'multipart/form-data'),
        'title' => array(
          '#markup' => '<a class="kcomment"><p>Ma réponse</p></a>',
        ),
        'compid' => array(
          '#type' => 'hidden',
          '#value' => $arg,
        ),
        'body' => array(
          '#type' => 'text_format',
          '#size' => 20,
          '#format' => 'full_html',
          '#attributes' => array(
            'style' => 'width: 90%',
          ),
          '#required' => TRUE,
        ),
        'fileforum' => array(
          '#type' => 'managed_file',
          '#attributes' => array(
            'style' => 'width: 90%; margin-top: 15px;',
          ),
          '#title' => t('AJOUTER UNE PIÈCE JOINTE'),
          '#upload_location' => 'public://forum_files/',
        ),
        
        'submit' => array(
          '#type' => 'submit',
          '#value' => $this->t('VALIDER'),
          '#ajax'  => array(
            'url' => Url::fromRoute('kozon.save.comment_parent'),
          ),
        ),

        'annuler' => array(
          '#type' => 'button',
          '#value' => $this->t('ANNULER'),
          '#ajax'  => array(
            'url' => Url::fromRoute('kozon.hide.comment_grandparent',['arg'=>$arg]),
          ),
        ),
    );
    return $form;
  }

  /**
   * {@inheritdoc}
   * @todo obsolete?
   */
  public function validateForm(array &$form, FormStateInterface $form_state) {
  }

  /**
   * {@inheritdoc}
   * @todo obsolete?
   */
  public function submitForm(array &$form, FormStateInterface $form_state) {
  }

}
